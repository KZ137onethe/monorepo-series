...
为了方便管理版本（多包）和记录不同子包的变更日志，我们可以使用`changesets`

### 参考

官网：[Changesets](https://changesets.dev/)
...
* 初始化
  ...
  他会生成一个.changeset文件夹，会包含changeset的配置文件`config.json`和发布版本前存储日志的文件`README.md`，如：
...
* 配置说明
  ...
  `config.json`的配置请参考：[配置信息](https://changesets.dev/guide/config)
...
* 使用
  ...
  1. 变更日志
     ...
     可以使用cli命令：`npx changeset`或者`pnpm changeset`
     ...
     ![use-step-01](../static/img/03-02-002.png)
     ...
     * Patch
     ...
     讲人话就是比如：当前仓库根目录的`package.json`的`version`为`1.0.0`，
     ...
     ![use-step-01.1](..\static\img\03-02-003.png)

     按回车键确认描述。

     ![use-step-01.2](..\static\img\03-02-004.png)

     最后一步，它会问你：Is this your desired changeset?

     也就是"这是你想要的改变吗？"，如果确定会进行下一步骤，如果取消那么上面的步骤作废。

     变更日志会在`.changeset`目录下生成一份MarkDown文档，记录了这次的变更，如：

     ![use-step-01.3](../static/img/03-02-005.png)

     MarkDown的名称是随机的，记录的是：修改的子包名、变更的方式和此次变更内容。

     > 🧐注意：变更日志的cli命令可以执行多次，分别生成不同的MarkDown文档

  2. 版本控制

     根据变更日志的MarkDown文件，变更子包的版本（`package.json`中的`version`）。其次将记录的变更日志在各自的子包生成`CHANGELOG.md`，将变更内容添加到这个MarkDown文档中，若已存在`CHANGELOG.md`，则内容追加。可以参考：[Versioning](https://changesets.dev/guide/versioning-and-publishing#versioning)
     ...
     > 🧐注意：如果changesets的配置文件配置了`"commit": true`，在执行上面cli命令后会自动提交代码；如果是`false`，则需要手动把代码提交到远程仓库，才能在仓库中体现。
